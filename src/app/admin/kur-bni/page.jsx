...
const pathUploadIo =
  "https://cepatsehats.com/api/v3/cs/uploads/izin-operasional/";
...
const dataKurBni = () => {
  ...
  const downloadAllFiles = async () => {
    ...
    try {
      for (const item of filteredData) {
        ...
        zip.file(
          `NPWP_${item.clinic_name}.${item.extensionFileNpwp}`,
          blobNpwp
        );

        const responseIzin = await fetch(
          cors + encodeURIComponent(item.izin_operasional)
        );
  ...
  const columns = [
    ...
    },
    {
      name: "KTP",
      selector: (row) => (
        <a href={row.ktp}>{row.ktp !== null ? `KTP_${row.clinic_name}` : ""}</a>
      ),
      sortable: true,
    },
    {
      ...
      selector: (row) => (
        <a href={row.npwp}>
          {row.npwp !== null ? `NPWP_${row.clinic_name}` : ""}
        </a>
      ),
      sortable: true,
    },
    {
      ...
      selector: (row) => (
        <a href={row.izin_operasional}>
          {row.izin_operasional !== null ? `IO_${row.clinic_name}` : ""}
        </a>
      ),
      sortable: true,
    },
  ...
  return (
    <>
      ...
      <div>
        <NavbarAdmin />
        <div className="row mt-4 pb-3 mx-14">
